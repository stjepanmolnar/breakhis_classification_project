...
if continuous_training:
    model = load_model("models/efficientnet_fusion_model_final.keras", custom_objects={"L2NormalizationLayer": L2NormalizationLayer})
    ...
    EfficientNetB3_model = EfficientNetB3(weights='imagenet', include_top=False, input_tensor=x)

    EfficientNetB3_model.trainable = False
    EfficientNetB3_model.summary()

    # Extract intermediate layers
    layer_names = ['block3a_activation', 'block4a_activation', 'block6a_activation']
    intermediate_outputs = [EfficientNetB3_model.get_layer(name).output for name in layer_names]
    # Create a new model that outputs intermediate layers
    intermediate_model = Model(inputs=EfficientNetB3_model.inputs, outputs=intermediate_outputs)
...
model.save("models/efficientnet_fusion_model.keras")
...
model.save("models/efficientnet_fusion_model_final.keras")
